...
## Proyectos en curso
- **Kaizen ⚡ (Asana) — sistema de evolución continua de Leeloo:** creado 2026-07-04 con JP. Ciclo: captura → construye → vive → reflexiona. Se arma "poco a poco" a partir de brain dumps de JP (él vacía ideas desordenadas, yo las acomodo). **1er brain dump (2026-07-04): 6 ideas en Backlog** — (1) mapear capacidades de Leeloo, (2) conector navegador, (3) voz nueva TTS, (4) cultura de verificación/loops, (5) optimización de tokens/contexto, (6) editar reels IG/TikTok. **Siguiente paso pendiente:** priorizar/ordenar las 6 (la #1 es base que alimenta #2/#3/#6). Detalle, GIDs y decisiones en `memory/project_kaizen_asana.md`.
...
## Pendientes abiertos
- **🗓️ LUNES 6-JUL-2026 9am PT — Sesión formal del "puente"/navegador (= Kaizen idea #2, CONFIRMADA por JP).** Habilitar que Leeloo opere **LinkedIn, X, Instagram, etc.** vía el navegador/sesiones logueadas de JP, con "ciertas libertades" que él otorgue, **solo cuando lo pida**. Respaldo: evento en Google Calendar (id `fvqhptim57acp7lhom56mchi0k`) + cron interno. Traer PROPUESTA: conector/MCP tipo extensión-puente que conecte su Chrome real (con logins) a mi sesión remota, vs. "navegador aislado" (SIN sus logins); pasos del puente (Leeloo remota + navegador local); "libertades" por red (borrador vs. publicar directo). En v1 era Chrome con `--remote-debugging-port=9222` (CDP), obsoleto. Cambio de config → explicar paso a paso, confirmar acciones sensibles.
- **⚠️ TTS/audio (= Kaizen idea #3):** en v2 NO hay herramienta de generación de voz → los resúmenes "por audio" que prefiere JP caen a TEXTO hasta resolverlo. **DECISIÓN JP (2026-07-04): voz NUEVA desde cero** (no afinar la anterior de ElevenLabs); abierto a otra plataforma. Leeloo traerá comparativa (naturalidad, costo, e integración al entorno para poder mandar audio real por Telegram — ese es el reto, no solo clonar la voz).
