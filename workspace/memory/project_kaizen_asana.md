...
1. **Mapear cómo está creada Leeloo** (GID `1216278241906203`) — qué capacidades tengo / no tengo / podría tener y bajo qué implicaciones. Se entiende poco a poco cada que surja el tema. 1er dato: Obsidian NO instalado. Ligada: "Explorar Obsidian juntos" (GID `1216278349881262`).
   - **DECISIÓN 2026-07-20 — Obsidian bajado del foco:** al explorarlo, Pablo y yo concluimos que Obsidian es solo un VISOR/editor de markdown que correría en SU compu, y mi memoria YA es markdown que yo leo/escribo con mis herramientas → **NO me suma capacidades a mí**. Su único valor sería una ventana para que Pablo navegue mi "cerebro" como grafo (foto fija = copia que se desactualiza; espejo vivo = requiere el Puente). Pablo decidió dejarlo para más adelante. Lo ÚNICO que sí me ayudaría (sin instalar nada) es que yo adopte enlaces estilo `[[nota]]` dentro de mi propia memoria — pendiente opcional. **No re-proponer instalar Obsidian salvo que Pablo lo pida.**
...
6. **Editar reels IG/TikTok** (GID `1216278296386866`) — CapCut confirmado; prioridad velocidad-IA (reels casi listos) > control fino. Rol Leeloo: A) copiloto creativo, B) ffmpeg programático (sin GUI). Liga con #1 y #2.
   - **ESPECIFICACIÓN KLING + CapCut (2026-07-27):** herramienta de generación de video con IA elegida = **KLING** (clips/b-roll); edición/ensamblaje = **CapCut**. Se incorpora Kling **una vez que el Puente (F1·5) esté listo** → tarea marcada con **dependencia real de F1·5** (1216278242162994). **JP crea y paga la cuenta/suscripción.** Rol Leeloo = **copiloto creativo** (concepto, guion, prompts de Kling, subtítulos, copy, hashtags); JP hace el arrastre visual en CapCut. Próximo paso: cuando JP dé un tema, armar concepto + prompts para el primer video de prueba.
   - **NOTA Fal.ai (opción fase 2, 2026-07-27):** plataforma/API de inferencia para IA generativa de medios (motor por debajo, no app con GUI); da acceso vía API a modelos top (FLUX, Kling, Veo, Luma, Runway, Wan), pago por uso, orientada a devs. **No** se necesita para el flujo actual Kling app + CapCut; su valor es AUTOMATIZAR (que Leeloo dispare generaciones por API). Conecta con el Puente (F1·5) y Merlín (F2·7). Registrada como nota en la tarea F1·6.
...
8. **📧 Correo propio de Leeloo (envío de mails con CC a Pablo)** (GID `1216408680019470`) — ✅ **IMPLEMENTADO 2026-07-20**. Cuenta `leeloo.asistenteai@gmail.com` (adulto), 2 pasos ON, app-password cifrada en `.secrets/leeloo_gmail_apppw.key`, script `workspace/send_email.py` (SMTP **puerto 587 STARTTLS** — el 465 y el 25 están bloqueados en la caja). Regla: CC obligatorio a Pablo. Primer correo de prueba entregado OK. Confirmar antes de enviar salvo libertades.
