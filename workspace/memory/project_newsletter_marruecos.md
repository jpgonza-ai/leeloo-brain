...
- **⏰ ENVÍO FIJO A LAS 10:00 AM PT EN PUNTO, NO ANTES (Pablo, 2026-08-07 msg 1737).** El 2026-08-07 lo mandé ~9:32am (tras cerrar la selección de fotos con Pablo) y me pidió que quede fijo a las 10am. **Cómo aplicar:** disparar el `send_email.py` a las 10:00am PT exactas.
- **✅ ENVÍO AUTÓNOMO, SIN PEDIR OK (Pablo, 2026-08-28 msg 2290): "este debe salir sin que me preguntes".** El post semanal de WAMT sale SOLO a las 10 AM PT viernes; NO mandar la propuesta a Pablo para aprobación previa (él queda en CC y lo ve). **Cómo aplicar:** cada viernes preparo los 3 captions + elijo fotos reales sin repetir (revisar `REGISTRO_FOTOS_TAVO.md` por md5) y envío directo a Tavo con `send_email.py` (CC Pablo) a las 10am. Actualizar el registro al enviar y confirmar a Pablo por Telegram DESPUÉS. **Única excepción para tocarle la puerta:** que no tenga con qué armarlo (ej. banco de fotos reales sin repetir agotado). Este viernes 2026-08-28 sí le pregunté (banco casi agotado + sin foto de hammam) y me recordó la regla.
...
### ⛔ REGLA ANTI-REPETICIÓN DE FOTOS (Pablo, 2026-08-21)
Pablo notó que las fotos de referencia a Tavo se estaban **repitiendo** (dos ya reutilizadas: el patio de riad/alberca `maj_pool.jpg` en 07-31 y 08-14; el campamento Sahara `camp1.jpg` en 07-31 y 08-21). No confiarse en que "al final Tavo escoge".
- **Registro durable:** `newsletter_assets/ig_posts/REGISTRO_FOTOS_TAVO.md` — tabla de qué foto se mandó cada semana + **md5** (los nombres engañan; el md5 es la verdad). Tiene una lista de "huellas quemadas".
- **How to apply:** ANTES de elegir fotos cada viernes, abrir el registro; **no reutilizar ninguna md5 ya enviada**; al enviar, agregar las nuevas filas + huellas al registro. Verificar con `md5sum` contra la lista de quemadas.
- **Banco casi agotado:** ~11 fotos reales distintas y ya se usaron 10; la única real sin usar es `morocco_real/merzouga.jpg` (dunas Erg Chebbi). **Pendiente con Pablo:** cómo ampliar el banco de fotos REALES (él las cura/sube, o autoriza método para conseguir más). No usar IA para estos posts.
